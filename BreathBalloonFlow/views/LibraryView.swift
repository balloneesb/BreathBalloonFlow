...
struct LibraryView: View {
    ...
    // --- Added to save likes ---
    private func saveLikedTechniques() {
        let likedIDs = techniques.filter { $0.isLiked }.map { $0.id }
        UserDefaults.standard.set(likedIDs, forKey: "likedTechniques")
    }
    
    private func loadLikedTechniques() -> Set<String> {
        let likedIDs = UserDefaults.standard.stringArray(forKey: "likedTechniques") ?? []
        return Set(likedIDs)
    }
    ...
    var body: some View {
                                ...
                                TechniqueCard(
                                    ...
                                    onLikeTap: {
                                        if let index = techniques.firstIndex(where: { $0.id == technique.id }) {
                                            ...
                                            saveLikedTechniques() // Сохраняем лайки при изменении
                                        }
                                    }
                                )
                            }
        ...
        .onAppear {
            var loadedTechniques = loadTechniques()
            let likedIDs = loadLikedTechniques()
            for i in 0..<loadedTechniques.count {
                if likedIDs.contains(loadedTechniques[i].id) {
                    loadedTechniques[i].isLiked = true
                }
            }
